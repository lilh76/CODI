...
class ReplayBuffer:
    ...
    def _allocate(self, key, array):
        ...
        print(key, shape, self.max_n_episodes, self.max_path_length, self.n_agents, dim)
        self._dict[key] = np.zeros(shape, dtype=np.float32)

    def add_path(self, path):
        # path[key] shape: (path_length, n_agents, dim)
        '''
        observations (25, 3, 17)
        actions (25, 3, 1)
        rewards (25, 3, 1)
        labels (25, 3, 1)
        terminals (25, 3, 1)
        ...
        assert path_length <= self.max_path_length, (path_length, self.max_path_length)
