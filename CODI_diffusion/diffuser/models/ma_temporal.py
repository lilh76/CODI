...
class SharedConvAttentionDeconv(nn.Module):
    ...
    def __init__(
        ...
        self.returns_condition = returns_condition
        self.labels_condition = include_labels
        self.env_ts_condition = env_ts_condition
        ...
        self.net = TemporalUnet(
            ...
            dim_mults=dim_mults,
            returns_condition=returns_condition,
            labels_condition=include_labels,
            n_agents=n_agents,
            env_ts_condition=env_ts_condition,
            condition_dropout=condition_dropout,
            max_path_length=max_path_length,
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        # print('AAA', x[0, :, 0])
        # print('AAA', x.shape) # syn [bs, h, n_agents, obs_shape]
        # print('AAA', returns[0, 0, 0])
        # print('AAA', returns.shape) # syn [bs, 1, n_agents]
        ...
        labels = None
        # print(returns)
        # print(11111, self.returns_condition, self.labels_condition)
        if isinstance(returns, tuple) or isinstance(returns, list):
            if self.returns_condition and self.labels_condition:
                returns, labels = returns
                # print(returns.shape) # [bs, 1, n_agents]
                # print(labels.shape) # [bs, h2, n_agents, 1]
                # assert 0
            else:
                assert 0
        ...
        if self.returns_condition:
            ...
            returns_embed = self.net.returns_mlp(returns) # [bs, n_agents, e_dim]
            if use_dropout:
                # here use the same mask for all agents
        ...
        if self.labels_condition:
            # print(use_composition_tech)
            # print(compose_agent_idx)
            # assert 0
            assert labels is not None
            
            # labels = labels[:, :1, :, 0].repeat(1, self.n_agents, 1) # [bs, h2, n_agents, 1] -> [bs, 1, n_agents] -> [bs, n_agents, n_agents]
            # labels_embed = self.net.labels_mlp(labels) # [bs, n_agents, e_dim]
            # if use_dropout: # train: 1, syn: 0
            #     # Bernoulli(probs: 0.75)
            #     mask = self.net.mask_dist.sample(sample_shape=(labels_embed.size(0), labels_embed.size(1), 1)).to(labels_embed.device)
            #     labels_embed = mask * labels_embed
            # if force_dropout: # train: 0, syn: 0
            #     labels_embed = 0 * labels_embed
            # if use_composition_tech:
            #     mask = torch.zeros((labels_embed.size(0), labels_embed.size(1), 1)).to(labels_embed.device) # [bs, n_agents, 1]
            #     mask[:, compose_agent_idx] = 1
            #     labels_embed = mask * labels_embed
            # t = torch.cat([t, labels_embed], dim=-1)
            
            bs, _, n_agents, _ = labels.shape
            labels = labels[:, :1] # [bs, h2, n_agents, 1] -> [bs, 1, n_agents, 1]
            agent_ids = torch.arange(n_agents, device=labels.device)
            agent_onehot = F.one_hot(agent_ids, num_classes=n_agents)  # [n_agents, n_agents]
            agent_onehot = agent_onehot.unsqueeze(0).unsqueeze(0)  # [1, 1, n_agents, n_agents]
            agent_onehot = agent_onehot.expand(bs, 1, n_agents, n_agents)  # [bs, 1, n_agents, n_agents]
            labels = torch.cat([labels, agent_onehot], dim=-1)  # [bs, 1, n_agents, 1 + n_agents]
            labels = labels.repeat(1, n_agents, 1, 1) # [bs, n_agents, n_agents, 1 + n_agents]
            labels_embed = self.net.labels_mlp_1(labels) # [bs, n_agents, n_agents, e_dim]
            if use_dropout: # train: 1, syn: 0
                # Bernoulli(probs: 0.75)
                mask = self.net.mask_dist.sample(sample_shape=(labels_embed.size(0), labels_embed.size(1), labels_embed.size(2), 1)).to(labels_embed.device)
                # e.g. [1 0 1] mask the second agent
                # [bs, n_agents, n_agents, 1]
                labels_embed = mask * labels_embed
            if force_dropout: # train: 0, syn: 0
                labels_embed = 0 * labels_embed
                
            if use_composition_tech: # only True during syn
                mask = torch.zeros((labels_embed.size(0), labels_embed.size(1), labels_embed.size(2), 1)).to(labels_embed.device) # [bs, n_agents, n_agents, 1]
                mask[:, :, compose_agent_idx] = 1
                labels_embed = mask * labels_embed
                
            labels_embed = labels_embed.reshape(labels_embed.size(0), labels_embed.size(1), -1) # [bs, n_agents, n_agents, e_dim] -> [bs, n_agents, n_agents * e_dim]
            labels_embed = self.net.labels_mlp_2(labels_embed) # [bs, n_agents, e_dim]
        ...
        h = []
        x = x.reshape(x.shape[0] * x.shape[1], x.shape[2], x.shape[3]) # [bs * n_agents, obs_shape, horizon]
        t = t.reshape(t.shape[0] * t.shape[1], t.shape[2]) # [bs * n_agents, e_dim]
