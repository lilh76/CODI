...
class SequenceAugDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        label_model_hidden_dim: int = 256,
    ...
    def __getitem__(self, idx: int, agent_idx: Optional[int] = None):
        ...
        if self.use_action:
            # print(f"[DATASET] action shape: {actions.shape}, obs shape: {observations.shape}")
...
class SequenceLLMLabelsDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        n_agents: int = 2,
        horizon: int = 64,
        ...
        env_type: str = "d4rl",
        env: str = "hopper-medium-replay",
        use_llm_labels: str = True,
        task_dir: str = ''
    ):
        """
        restore the original x, rewards, labels from oar_{cnt}.npy and {llm/true}_labels_{cnt}.npy
        """

        self.batches_dir = f'diffuser/datasets/data/{task_dir}/batches'
        self.n_agents = n_agents
        self.horizon = horizon
        self.cnts = list(cnts)

        print("Loading collected label batches from batches_dir:", self.batches_dir)
        x_parts, rewards_parts, labels_parts = [], [], []

        for cnt in self.cnts:
            oar = np.load(os.path.join(self.batches_dir, f"oar_batch_{cnt}.npy"))
            labels = np.load(os.path.join(self.batches_dir, f"{'true' if not use_llm_labels else 'llm'}_labels_batch_{cnt}.npy"))

            bs, h2, lastdim = oar.shape
            feat_dim = (lastdim - 1) // n_agents

            feats = oar[..., :-1].reshape(bs, h2, n_agents, feat_dim)
            rewards = oar[..., -1:].reshape(bs, h2, 1)

            if labels.ndim == 2:  # [bs, n_agents]
                labels = labels[..., None]
            # [bs, n_agents, 1]
        ...
        self.x = torch.from_numpy(np.concatenate(x_parts, axis=0)).float()
        self.rewards = torch.from_numpy(np.concatenate(rewards_parts, axis=0)).float()
        self.labels = torch.from_numpy(np.concatenate(labels_parts, axis=0)).float()

        self.feature_dim = self.x.shape[-1]
        self.n_samples = self.x.shape[0]

        print(f"[SequenceLLMLabelsDataset] Loaded {self.n_samples} samples "
              f"from {len(self.cnts)} batches, feature_dim={self.feature_dim}")
