...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        self.labels_condition = labels_condition
        ...
        if self.labels_condition:
            self.labels_mlp_1 = nn.Sequential(
                nn.Linear(1 + n_agents, dim),
                act_fn,
                nn.Linear(dim, dim * 4),
                act_fn,
                nn.Linear(dim * 4, dim),
            )
            self.labels_mlp_2 = nn.Sequential(
                nn.Linear(n_agents * dim, dim),
                act_fn,
                nn.Linear(dim, dim * 4),
                act_fn,
                nn.Linear(dim * 4, dim),
