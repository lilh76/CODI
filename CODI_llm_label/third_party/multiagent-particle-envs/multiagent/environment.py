...
class MultiAgentEnv(gym.Env):
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        # print(action)  # [array([-0.12733555,  0.4820574 ], dtype=float32)] -> [x]

        if agent.movable:
            # physical action
            if self.discrete_action_input and isinstance(action[0], int):
                agent.action.u = np.zeros(self.world.dim_p)
                ...
                if action[0] == 4:
                    agent.action.u[1] = +1.0
            else:  # onehot or onehot logits or continous input
                if self.force_discrete_action:
                    d = np.argmax(action[0])
                    action[0][:] = 0.0
                    action[0][d] = 1.0
                if self.discrete_action_space and len(action[0]) == self.action_space[0].n:
    ...
    # reshape obs to form states
    def get_state(self):
        obs_n = []
        self.agents = self.world.policy_agents
        for agent in self.agents[:3]:
            obs_n.append(self._get_obs(agent))
        state = np.concatenate(obs_n, axis=-1)
        return state

    def get_state_size(self):
        return 3 * self.observation_space[0].shape[0]
