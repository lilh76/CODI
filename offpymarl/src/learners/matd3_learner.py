    ...
    def train(self, batch, t_env: int, episode_num: int):
        ...
        if (self.training_steps + 1) % self.actor_freq == 0:
            ...
            if "bc" in self.args.name: # matd3+bc
                ...
                pis_mask = mask.expand_as(pis)  # [bs, (T-1), n_agents, n_actions]

                pis = pis.reshape(-1, self.n_actions)
                actions_4bc_flat = actions_4bc.reshape(-1)  # [bs*(T-1)*n_agents]
                pis_mask_flat = pis_mask.reshape(-1, self.n_actions)  # [bs*(T-1)*n_agents, n_actions]

                bc_loss_per_sample = F.cross_entropy(
                    pis, 
                    actions_4bc_flat, 
                    reduction='none'
                )  # [bs*(T-1)*n_agents]

                sample_mask = pis_mask_flat[:, 0]  # [bs*(T-1)*n_agents]

                bc_loss = (bc_loss_per_sample * sample_mask).sum()
                bc_loss = bc_loss / (sample_mask.sum() + 1e-5)

                mask = mask.reshape(-1, 1)
                lmbda = self.args.td3_alpha / ((q * mask).abs().sum().detach() / (mask.sum() + 1e-5))
